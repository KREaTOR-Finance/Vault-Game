/* Raw devnet bootstrap for VaultCrack v1 program.
 *
 * Sends:
 *  1) initialize_global(skr_mint, backend_authority, treasury)
...
async function main() {
  ...
  const backendAuthority = arg('--backendAuthority');
  ...
  if (!backendAuthority) throw new Error('Missing --backendAuthority');
  ...
  // initialize_global(skr_mint, backend_authority, treasury)
  const initData = concat(
    ixDiscriminator('initialize_global'),
    pkBytes(skrMint),
    pkBytes(backendAuthority),
    pkBytes(treasury)
  );
