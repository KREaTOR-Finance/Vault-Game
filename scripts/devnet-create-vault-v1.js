/* Create a single VaultCrack v1 vault on devnet (raw instruction encoding).
 *
 * v1 create_vault args:
 *   end_ts: i64
 *   pin_len: u8
 *   is_system: bool (u8)
 *   prize_amount: u64
 ...
 *
 * Usage:
 *  node scripts/devnet-create-vault-v1.js \
 *    --programId <PROGRAM_ID> \
 *    --wallet <KEYPAIR_JSON> \
 *    --feeMint <MINT> \
 *    --creatorFeeTokenAccount <TOKEN_ACCOUNT_FOR_MINT> \
 *    --pinLen <3|4|6|8> \
 *    --prize <u64> \
 *    --baseFee <u64> \
 *    --feeStep <u64> \
...
function readGlobalVaultCount(gsBytes) {
  // GlobalState v1: disc(8) + authority(32) + backend_authority(32) + treasury(32) + skr_mint(32) + vault_count(u64)
  const off = 8 + 32 + 32 + 32 + 32;
...
async function main() {
  ...
  const baseFee = BigInt(arg('--baseFee', '1'));
  const feeStep = BigInt(arg('--feeStep', '1'));
  const isSystem = arg('--isSystem', 'false') === 'true';
  const setMega = arg('--setMega', 'false') === 'true';

  if (!walletPath || !feeMint || !creatorFeeTokenAccount) throw new Error('Missing required args');
  ...
  if (!gsInfo) throw new Error('GlobalState missing; run devnet-init-raw-v1 first');
  ...
  console.log('creating vaultPda', vaultPda.toBase58(), 'vaultCount', vaultCount.toString(), 'pinLen', pinLen, 'isSystem', isSystem);
